...
const Appointments = async (props: { searchParams?: { [key: string]: string | undefined } }) => {
    ...
    const page = (searchParams?.p || "1") as string; // p = page number
    const searchQuery = searchParams?.q || ""; // q = search query
    ...
    if (userRole === "admin" || (userRole === "doctor" && id || (userRole === "nurse" && id))) {
        ...
        queryId = userId;
    } else if (userRole === "nurse") { // because nurse can't have appointments (only view them)
        queryId = undefined;
    }
    ...
    const renderItem = (item: DataProps) => {
        ...
        return <tr key={item?.id} className='border-gray-200 even:bg-slate-50 text-sm hover:bg-slate-50'>
            <td className='flex items-center gap-2 md:gap-4 py-2 xl:py-4'>
                <ProfileImage url={item?.patient?.img!} name={patient_name} bgColor={item?.patient?.colorCode!} />
                <div>
                    <h3 className='font-semibold uppercase'>{patient_name}</h3>
                    <span className='text-xs md:text-sm capitalize'>
                        {item?.patient?.gender.toLocaleLowerCase()}
                    </span>
                </div>
            </td>
            ...
            <td className='hidden  items-center py-2 md:table-cell'>
                <div className='flex items-center gap-2 md:gap-4'>
                    <ProfileImage url={item.doctor?.img!} name={item.doctor?.name!} bgColor={item?.doctor?.colorCode!} textClassName='text-black' />
                    <div>
    ...
    // console.log(data);
    return <div className='bg-white rounded-xl p-2 md:p-4 2xl:p-6'>
